import { parse, traverse, Group } from 'regexp-tree';
import { ast2str, str2ast, bool2str } from '../runtime/utils';
import { backSpaceCharSet, singletonCharSet, emptyCharSet, charRangeCharSet, unionCharSets, charSetToStr, dotCharSet, digitCharSet, whiteSpaceCharSet, nonWhiteSpaceCharSet, wordCharSet, nonWordCharSet, controlCharSet, nonDigitCharSet } from '../runtime/CharSet';
...
function genCharacterSetMatcher (ch_set, invert, ctx) { 
  var ch_set_str = charSetToStr(ch_set); 
  var invert_str = bool2str(invert); 
  var ret_str    = `characterSetMatcher(${ch_set_str}, ${invert_str}, ${ctx2str(ctx)})`; 
  return str2ast(ret_str); 
}
...
function genAssertionMatcher(ctx, re) { 
  ...
  switch (re.kind) { 
    ...
    default: throw new Error("genAssertionMatcher: unsupported assertion");
  }   
  ...
  switch (char_ast.kind) {
    ...
    case "meta": 
      switch (char_ast.symbol) {
        ...
        case "\v" : 
        case "\f" :
        case "\r" : return singletonCharSet (char_ast.codePoint);
  
        case undefined:
          // Character class escape
          switch (char_ast.value) {
            case "\\d": return digitCharSet (); 
            case "\\D": return nonDigitCharSet (); 
            case "\\s": return whiteSpaceCharSet ();
            case "\\S": return nonWhiteSpaceCharSet (); 
            case "\\w": return wordCharSet (); 
            case "\\W" : return nonWordCharSet ();  
            default: throw new Error(`Unsupported character class escape: ${char_ast.symbol}. value: ${char_ast.value}`);
          } 
               
        default: throw new Error(`Unsupported meta symbol meta-switch: ${char_ast.symbol}. value: ${char_ast.value}`); 
      }
...
function evaluateClassRanges(ranges, ctx) { 
  if (ranges.length === 0) {
    ...
    var compiled_ranges = ranges.map (function (x) {
      switch (x.type) {
        case "Char": return charAst2CharSet(x, ctx); 
        ...
        default: throw new Error ("Class Range Unsupported");
      }
    }); 
...
function compile (ctx, re) {
  ...
  switch (re.type) {
    ...
    case "Char": 
      ...
      var r = genCharacterSetMatcher (c_char_set, false, ctx); 
      return r;
    ...
    case "CharacterClass": 
      ...
      return genCharacterSetMatcher(ret_ranges, re.negative === true, ctx); 
    ...
    default: throw ("not supported yet: " + re.type); 
  }
}
